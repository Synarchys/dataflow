...
proc getColumns(pg: DbConn, dbName, dbTable: string): JsonNode =
  #echo fmt"==> GET column: {dbTable}" 
  let rows = pg.getAllRows(sql"""
    select to_json(k) 
      from (select array_to_json(array_agg(row_to_json(j))) as columns 
          from (select column_name, data_type from information_schema.columns 
             where  table_schema = 'public' and  table_catalog = ? and table_name= ?) j) k
    """, dbName, dbTable)
  if rows[0].len > 0:
...
proc getSchema*(pg: DbConn, dbName: string): JsonNode =
  ...
  """, db_name)
  for r in rows:
    result["tables"][r[0]] = pg.getColumns(db_name, r[0])
    

proc get_data*(pg: DbConn, jq: JsonNode): JsonNode =
...
proc post_data*(pg: DbConn, d: JsonNode): JsonNode =
...
proc delete_data*(pg: DbConn, jq: JsonNode): JsonNode =
...
proc put_data*(pg: DbConn, d: JsonNode): JsonNode =
...
# proc getColumns(pg: DbConn, db_name, db_table: string): seq[DBColumn] =
...
# proc getSchema*(pg: DbConn, db_name: string): seq[DBTable] =
...
#   """, db_name)
#   for r in rows:
#     let cols = pg.getColumns(db_name, r[0])
...
#   pg = c
#   db_schema = get_schema(schema_name)


# proc get_data*(jq: JsonNode): JsonNode =
#   ## Returns data from a table in JSON format, it takes key/values from
#   ## WebContext to generate the query.
#   ## At this time only equality is tested.
#   #echo $ctx.request.paramTable
